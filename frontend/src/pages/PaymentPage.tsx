...
const PACKAGES = [
  { id: 'classic', name: 'Classic', price: 950 },
  { id: 'premier', name: 'Premier', price: 1250 },
];

// Default viewing add-on amount
const DEFAULT_ADDON_AMOUNT = 300;

const PaymentPage = () => {
  // State for form values
  const [selectedPackage, setSelectedPackage] = useState<string>('');
  const [viewingAddonEnabled, setViewingAddonEnabled] = useState<boolean>(false);
  const [viewingAddonAmount, setViewingAddonAmount] = useState<number>(DEFAULT_ADDON_AMOUNT);
  ...
  const calculateTotal = (): number => {
    ...
    const addonAmount = viewingAddonEnabled ? viewingAddonAmount : 0;
    return packagePrice + addonAmount;
  };
  ...
  // Handle viewing add-on toggle
  const handleAddonToggle = (e: ChangeEvent<HTMLInputElement>) => {
    setViewingAddonEnabled(e.target.checked);
  };
  
  // Handle viewing add-on amount change
  const handleAddonAmountChange = (e: ChangeEvent<HTMLInputElement>) => {
    const value = parseFloat(e.target.value);
    if (!isNaN(value) && value >= 0) {
      setViewingAddonAmount(value);
    } else if (e.target.value === '') {
      setViewingAddonAmount(0);
    }
  };
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    if (viewingAddonEnabled && (isNaN(viewingAddonAmount) || viewingAddonAmount < 0)) {
      setError('Please enter a valid amount for the viewing add-on');
      return;
    }
    ...
    try {
      const response = await fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:3000'}/api/create-checkout-session`, {
        method: 'POST',
        headers: {
        ...
        body: JSON.stringify({
          ...
          viewingAddonEnabled,
          viewingAddonAmount: viewingAddonEnabled ? viewingAddonAmount : 0,
        }),
      });
  ...
  return (
    <div className="payment-container">
      <div className="payment-header">
        ...
        <h1>Where Can I Find A Home — Card Payment</h1>
      </div>
        ...
        <div className="form-group">
          <div className="addon-toggle">
            <input
                type="checkbox"
                id="viewingAddon"
                checked={viewingAddonEnabled}
                onChange={handleAddonToggle}
                className="toggle-checkbox"
            />
            <label htmlFor="viewingAddon" className="toggle-label">
              Viewing add-on
            </label>
          </div>
